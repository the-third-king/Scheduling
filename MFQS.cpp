...
*/
void rts(queue<Process> &processQueue, int mode) {
	...
	while (!processQueue.empty()) {
		...
		if (i >= processQueue.front().arrivalTime && i + processQueue.front().burstTime <= processQueue.front().deadline ) {
			Process process = processQueue.front();
			cout << "Process ID " << process.processNum << " has started."  << endl;
			processQueue.pop();
			int burst = 0;
			while (burst < process.burstTime) {
				//process going through burst 
				burst++;
				i++;
			}
		} else if (i >= processQueue.front().arrivalTime) {
			if (mode == 0) {
				break;
			}
			processQueue.pop();
